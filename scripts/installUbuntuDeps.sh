...
install_apt_deps(){
  ...
  sudo apt-get install -qq git make gcc-5 g++-5 libssl-dev cmake openjdk-6-jre libglib2.0-dev pkg-config libboost-regex-dev libboost-thread-dev libboost-system-dev liblog4cxx10-dev rabbitmq-server mongodb curl libboost-test-dev -y
