...
let getPublisherUserIdFromStream = function (stream) {
    if (stream.getAttributes() && stream.getAttributes()['userId']) {
        return stream.getAttributes()['userId'];
    } 
    return stream.getID();
}

let createTabs = function(remoteStreams) {
  let tabNode = document.getElementById('tabs');
  for (let id in remoteStreams) {
    let userId = getPublisherUserIdFromStream(remoteStreams[id]);
...
var initChart = function (stream, subId, userId) {
    ...
    let publisherId = getPublisherUserIdFromStream(stream);
    let chartTitle = userId === undefined? `pub: ${publisherId},\nsub: ${subId}`: `${publisherId}->${userId}`; 
...
let getOrCreateChart = function(stream, subId, userId) {
  let pubId = stream.getID();
  let chart;
  if (!charts.has(pubId)) {
    return undefined;
  ...
  if (!charts.get(pubId).has(subId)) {
    chart = {
      seriesMap: {},
      chart: initChart(stream, subId, userId)
    };
    charts.get(pubId).set(subId, chart);
  } else {
...
var updateSeriesForKey = function (stream, subId, userId, key, spatial, temporal, valueX, valueY, 
  pointName = undefined, isActive = true) {
    let chart = getOrCreateChart(stream, subId, userId);
    ...
    room.getStreamStats(stream, function(data) {
        for (let i in data) {
            if (i !== 'publisher') {
                let subId = i;
                let selectedLayers = '';
                let userId = data[i].metadata.userId;
                ...
                if (qualityLayersData !== undefined) {
                    ...
                    for (var spatialLayer in qualityLayersData) {
                        for (var temporalLayer in qualityLayersData[spatialLayer]) {
                            ...
                            updateSeriesForKey(stream, subId, userId, key, spatialLayer, temporalLayer, 
                ...
                if (totalBitrate) {
                    updateSeriesForKey(stream, subId, userId, 'Current Received', undefined, undefined, 
                      date, totalBitrate, selectedLayers);
                }

                updateSeriesForKey(stream, subId, userId, 'Estimated Bandwidth', undefined, undefined, 
                  date, bitrateEstimated);
                updateSeriesForKey(stream, subId, userId, 'Padding Bitrate', undefined, undefined, 
                  date, paddingBitrate);
                updateSeriesForKey(stream, subId, userId, 'Rtx Bitrate', undefined, undefined, 
                  date, rtxBitrate);
            }
        }
    });
